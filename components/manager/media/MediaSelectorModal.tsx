...
const MediaSelectorModal: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            // Fetch both in parallel for a faster UI
    ...
    useEffect(() => {
        if (isOpen) {
            fetchData();
            // Pre-fill selection based on what's currently on the product
    ...
    const handleConfirmSelection = () => {
        if (onSelect) {
            if (selectedAsset) {
                ...
                const coverImage = selectedCollection.media_assets?.[0]?.public_url || 'https://via.placeholder.com/400?text=No+Image';
                onSelect({
                    imageUrl: coverImage,
            ...
            <div className="flex flex-col h-[70vh]">
                ...
                <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                    <div className="text-xs text-slate-500">
                        ...
                        {!selectedAsset && !selectedCollection && 'No media selected.'}
                    </div>
